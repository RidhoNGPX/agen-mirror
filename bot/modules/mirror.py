...
from bot import Interval, INDEX_URL, VIEW_LINK, aria2, dispatcher, DOWNLOAD_DIR, \
...
class MirrorListener:
    ...
    def onUploadComplete(self, link: str, size, files, folders, typ, name: str):
        ...
        if self.isLeech:
            msg += f'\n<b>Elpased : </b><code>{get_readable_time(seconds=ts-int(self.message.date.timestamp()))}</code>'
            msg += f'\n<b>User: </b>{self.tag}(<code>{self.message.from_user.id}</code>)'
            if typ != 0:
