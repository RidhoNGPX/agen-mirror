...
from bot import aria2, app, LOGGER, DOWNLOAD_DIR, LEECH_SPLIT_SIZE, EQUAL_SPLITS
from bot.helper.agenhelper import post_to_telegraph
...
def mediainfo(path, name):
    try:
        result = check_output(["mediainfo", path]).decode('utf-8')
        title = f"📄 AgenMirror_Mediainfo"
        body_text = f"""
